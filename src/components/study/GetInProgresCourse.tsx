...
const GetInProgresCourse = () => {
  ...
  useEffect(() => {
    ...
    const getCourseStatus = async () => {
      try {
        const response = await axiosPrivate.get(`/course-status/user/${session?.user.id}`).then((res) => {
  ...
  return (
    ...
    <InProcessingCard language={ELanguages.C} exercises={82} onClick={() => {}} /> */}
    {courseStatusList.map((course) => (
       <InProcessingCard key={course._id} language={course.courseId.language} exercises={course.courseId.totalExercises} progress={course.progress} onClick={() => {
