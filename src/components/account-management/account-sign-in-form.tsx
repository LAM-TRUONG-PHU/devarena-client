...
const accountSignInFormSchema = z
  .object({
    "current-password": z.string().min(0).optional(),
    "new-password": z
      .string()
...
export default function AccountSignInForm() {
  ...
  const hasChangedAccountSignIn = Object.keys(defaultValuesAccountSignIn).some(
    (key) =>
      watchFieldsAccountSignIn[
        key as keyof typeof defaultValuesAccountSignIn
      ] !==
      defaultValuesAccountSignIn[key as keyof typeof defaultValuesAccountSignIn]
  );

  async function accountSignInOnSubmit(
    data: z.infer<typeof accountSignInFormSchema>
  ) {
              ...
              <div className="grid grid-cols-2 gap-2 !mt-10">
                {hasChangedAccountSignIn && (
                  <Button
                    ...
                    size="account"
                    onClick={() =>
                      accountSignInForm.reset(defaultValuesAccountSignIn)
                    }
                  >
                    CANCEL
                  </Button>
